    ...
    <Link href={`/article/${article.slug}`} style={{ display: "block", height: "100%" }}>
      <article className="card-hover" style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 18, overflow: "hidden", height: "100%", boxShadow: "0 8px 24px rgba(15,23,42,.06)" }}>
        ...
        <div style={{ padding: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 900, color: "#ea580c", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.06em" }}>
            {article.category || "Article"}
          </div>
          <h2 style={{ fontSize: 17, fontWeight: 900, lineHeight: 1.4, margin: 0, color: "#0f172a" }}>{article.title}</h2>
          {article.meta_desc && (
            <p style={{ color: "#64748b", fontSize: 13, lineHeight: 1.6, marginTop: 10 }}>{article.meta_desc}</p>
          )}
