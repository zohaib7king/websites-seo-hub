...
const templates = {
  modern: { label: "Modern Gulf", accent: "#6d28d9", sidebar: false },
  executive: { label: "Executive", accent: "#1e1b4b", sidebar: true },
  clean: { label: "Clean ATS", accent: "#0ea5e9", sidebar: false },
};
...
function CvPreview({ cv, template }) {
  ...
  if (config.sidebar) {
    return (
      <div id="cv-preview" style={{ background: "#fff", color: "#111827", minHeight: 980, boxShadow: "0 25px 70px rgba(15,23,42,.14)", borderRadius: 18, overflow: "hidden", display: "grid", gridTemplateColumns: "230px 1fr" }}>
  ...
  return (
    <div id="cv-preview" style={{ background: "#fff", color: "#111827", minHeight: 980, boxShadow: "0 25px 70px rgba(15,23,42,.14)", borderRadius: 18, padding: 34 }}>
      {content}
    </div>
  );
}
      ...
      <div className="cv-grid" style={{ display: "grid", gridTemplateColumns: "430px 1fr", gap: 26, alignItems: "start" }}>
        ...
        <div className="cv-preview-wrap" style={{ position: "sticky", top: 90 }}>
          <div className="no-print" style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }}>
            ...
            <button onClick={() => window.print()} className="career-btn career-btn-primary">Print / Save PDF</button>
          </div>
